...
Terraformを使用したアウトバウンド接続のないプライベートネットワークへのMicrosoft SQL Serverの導入は、us-east-1（北バージニア）リージョンでのみサポートされます。



== PostgreSQLのサポート

PostgreSQLは、Amazon CloudWatchやプライベートネットワークでの展開をサポートしていません。
