...
image:diagram-SAZ-database-architecture.png["単一のリージョンにAmazon FSx for NetApp ONTAPのアベイラビリティゾーンを1つ導入したスタンドアロンアーキテクチャの図"]

.複数のアベイラビリティゾーン
次の図は、単一のリージョンにフェイルオーバークラスタインスタンス（FCI）クラスタを含む2ノードのハイアベイラビリティ（HA）アーキテクチャを示しています。

image:diagram-MAZ-database-architecture.png["単一のリージョンにフェイルオーバークラスタインスタンスクラスタを配置した2ノードの高可用性アーキテクチャの図"]
